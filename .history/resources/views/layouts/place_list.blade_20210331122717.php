        ...
        <div class="col-8">
            @foreach ($places as $place)
                <div class="m-4" style="background-color: wheat">
                    ...
                    <div>
                        <div class="text-center m-2">投稿者</div>
                        <a class="mx-2" href={{ route('user.show', ['user' => $place->user->id]) }}>{{ $place->user->name }}</a>
                    </div>

                    <div>
                        <div class="text-center m-2">場所の名前</div>
                        <a class="mx-2" href={{ route('place.show', ['id' => $place->id]) }}>
                            {{ $place->name }}
                        </a>
                    </div>
                    <div>
                        <div class="text-center m-2">住所</div>
                        <div class="mx-2">{{ $place->address }}</ぢ>
                    </div>
                    <div>
                        <div class="text-center">コメント</div>
                        <div class="mx-3">{{ $place->comment }}</ぢ>
                    </div>
                    <div>
                        <div class="text-center">tag</div>
                        @foreach ($place->tags as $tag)
                            <div class="mx-2">{{ $tag->name }}</ぢ>
                        @endforeach
                    </div>
                    <div>
